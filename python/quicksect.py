...
class IntervalNode( object ):
    ...
    def contains( self, start, end, report_func ):
        if start >= self.start and end <= self.end: report_func( self )
        if self.left and start < self.left.maxend:
            self.left.contains( start, end, report_func )
        if self.right and end > self.start:
            self.right.contains( start, end, report_func )
